...
def analytics(municipality_id: Optional[int] = None, date_from: Optional[str] = None, date_to: Optional[str] = None):
    ...
    if date_from or date_to:
        def _parse(d):
            try:
                return dt.datetime.fromisoformat(str(d)[:10]).date()
            except Exception:
                return None
        df_dates = df.copy()
        df_dates["__d"] = df_dates["date"].apply(_parse)
        if date_from:
            try:
                dfrom = dt.date.fromisoformat(str(date_from)[:10])
                df_dates = df_dates[df_dates["__d"] >= dfrom]
            except Exception:
                pass
        if date_to:
            try:
                dto = dt.date.fromisoformat(str(date_to)[:10])
                df_dates = df_dates[df_dates["__d"] <= dto]
            except Exception:
                pass
        df = df_dates.drop(columns=["__d"], errors="ignore")
...
async def generate_plan(category:str, payload: dict, request: Request):
    ...
    insights = {
        'count': int(len(df_cat)),
        'hotspots': (
            df_cat['address'].dropna().astype(str).value_counts().rename_axis('address').reset_index(name='count')
            .head(5).to_dict('records') if not df_cat.empty else []
        ),
...
def make_plan_text(category:str, municipality_name:str, insights:dict|None=None)->str:
    today = dt.date.today()
    ...
    # Build insights-based header
    stats = insights or {}
    top_addrs = stats.get('hotspots', [])
    top_topics = stats.get('topics', [])
    total_cnt = stats.get('count', 0)
    recent = stats.get('recent_dates', [])
    ...
    if total_cnt:
        lines += [
            "",
            f"Обращений по категории за период в базе: {total_cnt}",
            "Карта очагов (топ-адреса):" if top_addrs else "",
        ]
        for a in top_addrs[:5]:
            lines.append(f"- {a['address']} — {a['count']} обращений")
        if top_topics:
            lines.append("Ключевые темы из текстов:")
            for t in top_topics[:7]:
                lines.append(f"- {t}")
